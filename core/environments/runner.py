...
class Runner:
    def __init__(self, env, agent, opponent_model=None, opponent_agent=None, use_model_logic=True):
        ...
        self.learned_opponent_mode = isinstance(opponent_agent, PPOAgent)
    ...
    def run_one_episode(self) -> tuple[dict[str, list], float, float, bool]:
        """
        ...
        for name in self.env.agent_iter():
            ...
            if name == "player_0":
                p0_rewards.append(float(reward))
                ...
                if self.opp_agent is not None:
                    if self.learned_opponent_mode:
                        # Learned PPO opponent: keep the opponent blind to opponent-model predictions.
        ...
        if len(p0_rewards) <= 1:
            return trajectory, 0.0, 0.0, False

        # Match the original stage accounting: drop the first bookkeeping reward and
        # score the episode by the terminal reward discounted by the player_0 turn depth.
        p0_rewards = p0_rewards[1:]
        discounted_episode_reward = float(p0_rewards[-1] * (self.agent.gamma ** (len(p0_rewards) - 1)))
        terminal_reward = float(p0_rewards[-1])
        return trajectory, discounted_episode_reward, terminal_reward, True
    ...
    def collect_batch(self, num_episodes):
        ...
        while collected_episodes < num_episodes:
            traj, episode_reward, terminal_reward, is_valid = self.run_one_episode()
            if not is_valid:
                continue

            collected_episodes += 1
            avg_reward += (episode_reward - avg_reward) / collected_episodes
        ...
        win_rate = (win_count + 0.5 * tie_count) / collected_episodes
        return batch_tensors, avg_reward, win_rate, opp_loss, opp_acc
